...
echo --------
echo cli test
time eval $SCRATCH_DIR/venv-4/bin/aws s3 cp s3://$BUCKET/$FILE $SCRATCH_FILE
rm $SCRATCH_FILE
...
echo s3fs
cd $SCRATCH_DIR
wget https://github.com/s3fs-fuse/s3fs-fuse/archive/v1.80.tar.gz --no-check-certificate
tar xvf v1.80.tar.gz
...
do
    ...
    time eval $SCRATCH_DIR/venv-$VALUE/bin/python boto3_download.py $BUCKET $FILE $SCRATCH_FILE
    rm $SCRATCH_FILE
done
...
echo ----------
echo boto2 test
time eval $SCRATCH_DIR/venv-$VALUE/bin/python boto2_download.py $BUCKET $FILE $SCRATCH_FILE
rm $SCRATCH_FILE
